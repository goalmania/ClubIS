...
async function PresidenteReportContent() {
  ...
  const [
    ...
    supabase.from('partite').select('gol_fatti, gol_subiti').eq('stato', 'giocata'),
  ])
  const totE = entrate?.reduce((s, r) => s + Number(r.importo), 0) ?? 0
  const totU = uscite?.reduce((s, r) => s + Number(r.importo), 0) ?? 0
  const fmt = (n: number) => n.toLocaleString('it-IT', { style:'currency', currency:'EUR', maximumFractionDigits:0 })
  const V = partite?.filter(p => (p.gol_fatti ?? 0) > (p.gol_subiti ?? 0)).length ?? 0
  const P = partite?.filter(p => (p.gol_fatti ?? 0) === (p.gol_subiti ?? 0) && p.gol_fatti !== null).length ?? 0
  const S = partite?.filter(p => (p.gol_fatti ?? 0) < (p.gol_subiti ?? 0)).length ?? 0
  const rows = [
    ...
    { sez:'Sport', l:'Partite giocate', v:`${(partite?.length ?? 0)}` },
    ...
    <div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:24 }}>
        <div>
          ...
          <p style={{ fontSize:14, color:'var(--grigio-3)', marginTop:4 }}>{oggi.toLocaleDateString('it-IT', { month:'long', year:'numeric' })}</p>
        </div>
        <button className="btn btn-primary btn-sm" onClick={undefined}>🖨 Stampa</button>
      </div>
      {sezioni.map(sez => (
        <div key={sez} className="card" style={{ padding:0, overflow:'hidden', marginBottom:16 }}>
          ...
          {rows.filter(r => r.sez === sez).map(r => (
            <div key={r.l} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'13px 18px', borderBottom:'1px solid var(--grigio-6)' }}>
              ...
              <span style={{ fontSize:15, fontWeight:700, fontFamily:'var(--font-mono)', color:(r as any).c ?? 'var(--grigio)' }}>{r.v}</span>
            </div>
          ))}
        </div>
      ))}
...
export default async function PresidenteReportPage() {
  return (
    <ServerFeatureGate feature="report_mensile_automatico" featureLabel="Report Mensile">
      <PresidenteReportContent />
    </ServerFeatureGate>
  )
}
