...
export default function AllenatorePartitePage() {
  ...
  const load = useCallback(async () => {
    ...
    partite?.forEach((p: any) => {
      ...
      result[p.squadra_id].gf += gf
      result[p.squadra_id].gs += gs
  ...
  async function inserisci(forza = false) {
    if (!form.squadraId || !form.avversario.trim()) {
      setToast({ msg: 'Avversario e squadra obbligatori', tipo: 'error' }); return
    }
    setSalvando(true)
    if (!forza) {
      const { data: existing } = await supabase.from('partite')
        ...
        .gte('data_ora', `${form.data}T00:00:00`)
        .lte('data_ora', `${form.data}T23:59:59`)
    ...
    const { error } = await supabase.from('partite').insert({
      ...
      avversario: form.avversario.trim(),
      data_ora: `${form.data}T15:00:00`,
      gol_fatti: form.gf, gol_subiti: form.gs,
      casa_trasferta: form.ct, tipo: form.tipo,
      competizione: form.competizione.trim() || null,
      giornata: form.giornata ? parseInt(form.giornata) : null,
      stato: 'giocata',
    })
  ...
  const prossimePS   = prossime.filter((p: any) => squadrePS.some(s => s.id === p.squadra_id))
  const prossimeGiov = prossime.filter((p: any) => squadreGiov.some(s => s.id === p.squadra_id))
    ...
    <div style={{ animation: 'fadeIn 0.3s ease' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ fontSize: 14, color: 'var(--grigio-3)', marginTop: 4 }}>{sqIds.map(s => s.nome).join(', ')}</p>
        </div>
      ...
      {tab === 'risultati' && (
        <>
                ...
                <div key={gruppo.label} style={{ marginBottom: 16 }}>
                  ...
                  {gruppo.list.map((p: any) => (
                    <div key={p.id} className="card" style={{ padding: '14px 18px', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 16 }}>
                      ...
                      <Link href={`/dashboard/allenatore/convocazioni?partita=${p.id}`} className="btn btn-primary btn-sm">
                        Convocazioni →
                      </Link>
                    </div>
                  ))}
          ...
          {/* Per squadra — divise per gruppo */}
          {squadre.length === 0 ? (
            <div className="card" style={{ padding: 40, textAlign: 'center', color: 'var(--grigio-4)', fontSize: 13 }}>Nessuna squadra</div>
          ) : (
            <>
              {[
              ...
              <div key={sq.id} style={{ marginBottom: 24 }}>
                ...
                <div className="card" style={{ overflow: 'hidden', padding: 0 }}>
                  <div className="table-wrap">
                    <table>
                      <thead>
                        <tr><th>Data</th><th>Avversario</th><th>Esito</th><th>Risultato</th><th>Tipo</th><th>Giornata</th></tr>
                      </thead>
                      <tbody>
                        {sq.lista.length === 0 ? (
      ...
      {tab === 'statistiche' && (
        <>
          {squadre.length === 0 ? (
            ...
            const tot = sq.v + sq.p + sq.s
            if (tot === 0) return (
              <div key={sq.id} className="card" style={{ padding: '20px 24px', marginBottom: 16 }}>
            ...
            const mediaGF = (sq.gf / tot).toFixed(2)
            const mediaGS = (sq.gs / tot).toFixed(2)
              ...
              <div key={sq.id} style={{ marginBottom: 28 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 10, marginBottom: 14 }}>
                  {[
                    { l: 'Giocate', v: tot },
                    { l: 'Punti',   v: sq.v * 3 + sq.p, c: 'var(--verde)' },
                    { l: 'Media Gol +', v: mediaGF, c: 'var(--verde)' },
                    { l: 'Media Gol –', v: mediaGS, c: 'var(--rosso)' },
                  ...
                  <div className="card" style={{ padding: '12px 18px', display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div style={{ display: 'flex', gap: 4 }}>
                      {forma.map((e, i) => {
                        ...
                        const s = cfg[e]
                        return (
                          <span key={i} style={{ display: 'inline-flex', width: 22, height: 22, borderRadius: 4, alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, background: s.bg, color: s.c }}>
                            {e}
                          </span>
                        )
                      })}
      ...
      {tab === 'manuale' && (
        <div className="card" style={{ padding: 24, maxWidth: 640 }}>
          ...
          <div style={{ marginTop: 24, display: 'flex', gap: 10 }}>
            <button className="btn btn-primary" onClick={() => inserisci(false)} disabled={salvando || !form.avversario.trim() || !form.squadraId}>
        ...
        {duplicate && (
          <div style={{ padding: 20 }}>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              ...
              <button className="btn btn-primary btn-sm" onClick={() => inserisci(true)} disabled={salvando}>{salvando ? 'Salvo…' : 'Inserisci comunque'}</button>
            </div>
          </div>
        )}
      </Modal>
