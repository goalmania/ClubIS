...
DO $$ BEGIN CREATE TYPE cis_contesto_ruolo_notifiche AS ENUM (
  'presidente','segretario','team_manager','allenatore','medico','giocatore','famiglia','ufficio_stampa'
); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- Aggiunge ufficio_stampa all'enum notifiche se già esiste (idempotente)
ALTER TYPE cis_contesto_ruolo_notifiche ADD VALUE IF NOT EXISTS 'ufficio_stampa';
...
-- Aggiunge ufficio_stampa all'enum ruolo_utente (idempotente)
-- ALTER TYPE ... ADD VALUE non può stare dentro un DO block con EXCEPTION
ALTER TYPE ruolo_utente ADD VALUE IF NOT EXISTS 'ufficio_stampa';

DO $$ BEGIN CREATE TYPE tipo_evento_media AS ENUM (
  'intervista_tv','conferenza_stampa','intervista_radio',
  'podcast','photoshoot','altro'
); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN CREATE TYPE stato_evento_media AS ENUM (
  'da_confermare','confermato','annullato','completato'
); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN CREATE TYPE stato_brief_locandina AS ENUM (
  'bozza','inviato_grafico','in_lavorazione','completato'
); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- Calendario eventi media / interviste
CREATE TABLE IF NOT EXISTS eventi_media (
  id                  UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  club_id             UUID NOT NULL REFERENCES clubs(id) ON DELETE CASCADE,
  tipo                tipo_evento_media NOT NULL DEFAULT 'altro',
  data_ora            TIMESTAMPTZ NOT NULL,
  durata_minuti       SMALLINT NOT NULL DEFAULT 30,
  luogo               VARCHAR(255),
  emittente_testata   VARCHAR(200),
  soggetti_coinvolti  UUID[] DEFAULT '{}',
  stato               stato_evento_media NOT NULL DEFAULT 'da_confermare',
  note                TEXT,
  creato_da           UUID REFERENCES utenti(id) ON DELETE SET NULL,
  created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at          TIMESTAMPTZ NOT NULL DEFAULT NOW()
...
-- Guardia difensiva eventi_media: deve venire PRIMA degli indici su club_id
DO $$ BEGIN
  ALTER TABLE eventi_media ADD COLUMN IF NOT EXISTS
    club_id UUID REFERENCES clubs(id) ON DELETE CASCADE;
EXCEPTION WHEN undefined_table THEN NULL; END $$;

CREATE INDEX IF NOT EXISTS idx_eventi_media_club ON eventi_media(club_id, data_ora);
CREATE INDEX IF NOT EXISTS idx_eventi_media_stato ON eventi_media(stato);

-- Brief locandine eventi
CREATE TABLE IF NOT EXISTS brief_locandine (
  id                UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  club_id           UUID REFERENCES clubs(id) ON DELETE CASCADE,
  partita_id        UUID REFERENCES partite(id) ON DELETE SET NULL,
  evento_media_id   UUID REFERENCES eventi_media(id) ON DELETE SET NULL,
  data_evento       DATE NOT NULL,
  titolo_evento     VARCHAR(255) NOT NULL,
  campo_impianto    VARCHAR(255),
  ora_inizio        TIME,
  competizione      VARCHAR(150),
  logo_home_url     TEXT,
  logo_away_url     TEXT,
  colori_preferiti  VARCHAR(100),
  note_grafiche     TEXT,
  stato             stato_brief_locandina NOT NULL DEFAULT 'bozza',
  file_finale_url   TEXT,
  creato_da         UUID REFERENCES utenti(id) ON DELETE SET NULL,
  created_at        TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at        TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

-- Guardia difensiva brief_locandine: deve venire PRIMA dell'indice su club_id
DO $$ BEGIN
  ALTER TABLE brief_locandine ADD COLUMN IF NOT EXISTS
    club_id UUID REFERENCES clubs(id) ON DELETE CASCADE;
EXCEPTION WHEN undefined_table THEN NULL; END $$;

CREATE INDEX IF NOT EXISTS idx_brief_locandine_club ON brief_locandine(club_id, data_evento);

-- RLS
ALTER TABLE eventi_media ENABLE ROW LEVEL SECURITY;
ALTER TABLE brief_locandine ENABLE ROW LEVEL SECURITY;

DO $$ BEGIN
  DROP POLICY IF EXISTS eventi_media_access ON eventi_media;
  CREATE POLICY eventi_media_access ON eventi_media FOR ALL USING (
    club_id = my_club_id() OR is_super_admin()
  );
END $$;

DO $$ BEGIN
  DROP POLICY IF EXISTS brief_locandine_access ON brief_locandine;
  CREATE POLICY brief_locandine_access ON brief_locandine FOR ALL USING (
    club_id = my_club_id() OR is_super_admin()
  );
END $$;
...
CREATE TABLE IF NOT EXISTS accrediti (
  id                UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  club_id           UUID REFERENCES clubs(id) ON DELETE CASCADE,
  ...
  settore           VARCHAR(50),
  creato_da         UUID REFERENCES utenti(id) ON DELETE SET NULL,
  approvato_da      UUID REFERENCES utenti(id) ON DELETE SET NULL,
  created_at        TIMESTAMPTZ  NOT NULL DEFAULT NOW(),
  updated_at        TIMESTAMPTZ  NOT NULL DEFAULT NOW()
);

-- Guardie difensive: se la tabella esisteva da un run precedente parziale
-- senza queste colonne, questo le aggiunge. Se già presenti, IF NOT EXISTS
-- le salta. Se la tabella non esiste ancora, l'eccezione viene ignorata.
DO $$ BEGIN
  ALTER TABLE accrediti ADD COLUMN IF NOT EXISTS
    club_id UUID REFERENCES clubs(id) ON DELETE CASCADE;
EXCEPTION WHEN undefined_table THEN NULL; END $$;
...
DO $$ BEGIN
  ALTER TABLE accrediti ADD COLUMN IF NOT EXISTS stato VARCHAR(20) NOT NULL DEFAULT 'in_attesa';
EXCEPTION WHEN undefined_table THEN NULL; END $$;

CREATE INDEX IF NOT EXISTS idx_accrediti_club       ON accrediti(club_id);
CREATE INDEX IF NOT EXISTS idx_accrediti_partita    ON accrediti(club_id, partita_id);
CREATE INDEX IF NOT EXISTS idx_accrediti_stato      ON accrediti(stato);

-- RLS DISABILITATA: controllo club_id lato API
-- (coerente con il pattern esistente per tabelle operative)
